...
import { getAxisEmphasis } from "@/lib/axisEmphasis";
...
export default function MainAxisBars({ percentages }: { percentages: MainScores }) {
  return (
    <section aria-labelledby="main-axes-title" className="space-y-5 border-t border-slate-200 pt-5">
      ...
      <ul className="space-y-5">
        {axes.map((axis) => {
          const emphasis = getAxisEmphasis(percentages[axis.first], percentages[axis.second]);
          return (
            <li key={axis.first} className="space-y-2">
              <div className="grid grid-cols-2 gap-3 text-sm font-semibold sm:text-base">
                <p className={`min-w-0 text-blue-800 ${emphasis === "second" ? "font-medium opacity-65" : "opacity-100"}`}>{`${axis.firstLabel} ${percentages[axis.first]}%`}</p>
                <p className={`min-w-0 text-right text-amber-800 ${emphasis === "first" ? "font-medium opacity-65" : "opacity-100"}`}>{`${axis.secondLabel} ${percentages[axis.second]}%`}</p>
              </div>
              ...
              <div aria-hidden="true" className="h-3">
                <div className="flex h-full w-full overflow-hidden rounded-full bg-slate-100">
                  <span className={`h-full shrink-0 bg-blue-600 ${emphasis === "second" ? "opacity-65" : "opacity-100"}`} style={{ width: `${percentages[axis.first]}%` }} />
                  <span className={`h-full shrink-0 bg-amber-400 ${emphasis === "first" ? "opacity-65" : "opacity-100"}`} style={{ width: `${percentages[axis.second]}%` }} />
                </div>
              </div>
            </li>
          );
