...
export default function ResultImageCard({ result, character, brand = "/icon.png" }: { result: Pick<ScoringResult, "mainResult" | "displaySubTags">; character?: string; brand?: string }) {
  ...
  return (
    <div style={{ width: 1080, height: 1350, boxSizing: "border-box", padding: "48px 72px", background: "#f8fafc", color: "#0f172a", fontFamily: "NanumGothic", display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center", gap: 20 }}>
      ...
      <h1 style={{ margin: 0, fontSize: 68, lineHeight: "88px", fontWeight: 800, whiteSpace: "pre", flexShrink: 0 }}>{splitImageText(mainResult.name, 13)}</h1>
      ...
      <p style={{ margin: 0, fontSize: 36, lineHeight: "52px", flexShrink: 0, fontWeight: 600, whiteSpace: "pre" }}>{splitImageText(mainResult.summary, 26)}</p>
      <p style={{ margin: 0, fontSize: 28, lineHeight: "42px", flexShrink: 0, color: "#5B6472", display: "flex", flexWrap: "wrap", justifyContent: "center", columnGap: 8, width: "100%" }}>
        {mainResult.description.trim().split(/\s+/).map((word, index) => <span key={index} style={{ whiteSpace: "nowrap" }}>{word}</span>)}
      </p>
