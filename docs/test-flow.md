...
선택은 로컬 답변과 다음 문항 위치를 함께 저장한 뒤 즉시 이동합니다.
...
마지막 선택을 로컬에 저장하고 모든 답변의 서버 저장을 확인한 뒤 최종 답변 24개를 기존 `calculateScore`에 전달합니다.
...
2. 느린 네트워크에서도 선택 즉시 다음 문항으로 이동하는지 확인합니다.
3. 이전 이동 후 선택이 유지되고, 다른 선택지를 고르면 교체 후 즉시 다음 문항으로 이동하는지 확인합니다.
...
10. 24번째 답변 후 큐 저장과 complete 성공을 거쳐 결과가 표시되는지 확인하고 결과 페이지를 새로고침합니다.
