...
현재 문항에 머물며 다음 버튼을 활성화하고 백그라운드 큐에서 DB에 저장한다. 로컬 변경이 실패하면 DB를 호출하지 않는다.
DB 저장 중에도 선택·다음 버튼 이동이 가능하다. 실패 시 안내와 재시도 버튼을 표시하고 테스트 진행은 허용한다.
재시작 확정 시 기존 최신 답변 큐를 drain한 후 retry_click과 새 attempt 생성을 실행한다.
drain 중 중복 클릭을 막고 실패 시 기존 progress와 credential을 유지하여 재시도할 수 있다.
...
이어하기/재시작/최종 결과 보기에서 다시 전송한다. 정상 저장 중에는 별도 진행 안내를 표시하지 않는다.
